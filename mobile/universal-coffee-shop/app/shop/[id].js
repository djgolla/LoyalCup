...
const getClosedMessage = (shop) => {
  if (!shop) {
    return 'This shop is currently closed. Please check back later.';
  }
...
const ShopImage = ({ uri, style, resizeMode }) => {
  ...
  if (isSvgUrl(uri)) {
    return <SvgImage uri={uri} style={style} resizeMode={resizeMode} />;
  }
  ...
  return (
    <View style={{ marginBottom: 12, paddingHorizontal: 8 }}>
      <Text
        ...
        onTextLayout={(e) => {
          if (!needsExpand && e.nativeEvent.lines.length > 2) {
            setNeedsExpand(true);
          }
        }}
      >
  ...
  return (
    <View style={[styles.heroWrap, styles.heroNoBanner]}>
      ...
      <View style={styles.heroNoBannerContent}>
        {logoUrl ? (
          <View style={styles.heroGlowRing}>
            <View style={styles.heroLogoCircleNoBanner}>
              <ShopImage
                uri={logoUrl}
                style={styles.heroLogoImgNoBanner}
                resizeMode="contain"
              />
            </View>
          </View>
        ...
        {shopName ? (
          <Text style={styles.heroNoBannerName} numberOfLines={1}>
            {shopName}
          </Text>
        ) : null}
      </View>
...
export default function ShopDetailScreen() {
  ...
  const toggleMod = (groupId, option, isRadio) => {
    setSelectedMods((prev) => {
      ...
      const exists = current.find((selected) => selected.id === option.id);
  ...
  const handleAddToCart = () => {
    ...
    if (!user) {
      Alert.alert(
        ...
        'Create a free LoyalCup account to add items, place mobile orders, and earn rewards.',
        [
          {
            text: 'Not Now',
            style: 'cancel',
    ...
    const extraPrice = customizations.reduce(
      (sum, customization) =>
        sum + (parseFloat(customization.price_adjustment) || 0),
      0
    );
  ...
  const handleDirections = () => {
    ...
    Linking.openURL(
      `https://maps.apple.com/?q=${encodeURIComponent(shop.address)}`
    );
  };
  ...
  const modItemGroups = selectedItem
    ? getItemModGroups(selectedItem)
    : [];

  const modsExtraPrice = Object.values(selectedMods)
    .flat()
    .reduce(
      (sum, customization) =>
        sum + (parseFloat(customization.price_adjustment) || 0),
  ...
  const renderMenuItem = (item) => (
    ...
    >
      {item.image_url && (
        <ShopImage uri={item.image_url} style={styles.menuItemImage} />
      )}
      ...
      <View style={styles.menuItemContent}>
        ...
        <View style={styles.menuItemFooter}>
          ...
          <View
            style={[
              styles.addButton,
              (openStatus === false || orderingUnavailable) &&
                styles.addButtonDisabled,
            ]}
          >
            <Feather
              name={orderingUnavailable ? 'pause' : 'plus'}
              size={18}
              color="#FFF"
            />
          </View>
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      >
        ...
        <View
          style={[
            styles.shopInfo,
            hasBannerAndLogo && styles.shopInfoWithLogo,
          ]}
        >
          {openStatus !== null && (
            <View
        ...
        {offers.length > 0 && (
          <View style={styles.offersSection}>
            <Text style={styles.offersSectionLabel}>
              🏷️ CURRENT OFFERS
            </Text>
            ...
            <>
                ...
                return (
                  <View key={category.id} style={styles.catSection}>
                    ...
                    {category.description && (
                      <Text style={styles.catSectionDesc}>
                        {category.description}
                      </Text>
                    )}
              ...
              {itemsByCategory.uncategorized?.length > 0 && (
                <View style={styles.catSection}>
                  <Text style={styles.catSectionTitle}>
                    {categories.length ? 'Other items' : 'Menu'}
                  </Text>

                  <View style={styles.menuGrid}>
                    {itemsByCategory.uncategorized.map(renderMenuItem)}
                  </View>
                </View>
              )}
            </>
            ...
            <View style={styles.modalHeader}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.modalBasePrice}>
                  ${parseFloat(selectedItem?.base_price || 0).toFixed(2)}
                </Text>
              </View>
                ...
                modItemGroups.map((group) => {
                  ...
                  return (
                    <View key={group.id} style={styles.modGroup}>
                      ...
                      {(group.modifier_options || group.options || []).map(
                        (option) => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              style={[
                                styles.modOption,
                                selectedOption && styles.modOptionSel,
                              ]}
                              onPress={() =>
                                toggleMod(group.id, option, isRadio)
                              }
                            >
                              <Text
                                style={[
                                  styles.modOptName,
                                  selectedOption && styles.modOptNameSel,
                                ]}
                              >
                                {option.name}
                              </Text>
                              ...
                              <View style={styles.modOptRight}>
                                ...
                                <View
                                  style={[
                                    styles.modCheck,
                                    selectedOption && styles.modCheckSel,
                                  ]}
                                >
                                  {selectedOption && (
                                    <Feather
