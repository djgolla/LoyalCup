/**
 * Checkout — Square-quoted totals.
 *
 * Square is the source of truth for tax/final total:
 * - Checkout summary uses /api/v1/payments/quote
...
export default function CheckoutScreen() {
  ...
  const buildOrderItems = () => {
    ...
    const group = Object.values(itemsByShop)[0];
    if (!group) return [];
  ...
  const loadQuote = async () => {
    ...
    try {
      ...
      const q = await apiClient.post('/api/v1/payments/quote', {
  ...
  const displayTax = quote?.tax ?? 0;
  const displayDiscount = quote?.loyalty_discount ?? 0;
  const displayTotal = quote?.final_charge ?? quote?.total ?? 0;
  ...
  const handlePay = async () => {
    ...
    if (!shopLocationId) {
      Alert.alert('Shop Not Ready', "This shop hasn't finished setting up payments.");
      return;
    }
    ...
    if (quoteLoading) {
      Alert.alert('Calculating total', 'Give us a second to calculate the final total with Square.');
      return;
    }
  ...
  const processPayment = async (nonce) => {
    ...
    try {
      ...
      const { shopId } = Object.values(itemsByShop)[0];
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={{ paddingBottom: 200 }} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.card}>
              {items.map((item, idx) => (
                ...
                >
                  ...
                  <Text style={styles.itemPrice}>{money((parseFloat(item.price) || 0) * (item.quantity || 1))}</Text>
                </View>
              ))}
            </View>
          </View>
                ...
                <View style={styles.pointsToggleLeft}>
                  ...
                  <View>
                    <Text style={styles.pointsToggleTitle}>
                      {pointsToRedeem > 0
                        ? `${pointsToRedeem} pts → −${money(displayDiscount)}`
                        : 'Redeem Points'}
                    </Text>
                  ...
                  {!canRedeem ? (
                    ...
                    <>
                      ...
                      <View style={styles.stepperRow}>
                        <TouchableOpacity
                          style={[styles.stepperBtn, pointsToRedeem <= 0 && styles.stepperBtnDis]}
                          onPress={dec}
                        ...
                        <TouchableOpacity
                          style={[styles.stepperBtn, pointsToRedeem >= maxRedeemable && styles.stepperBtnDis]}
                          onPress={inc}
        ...
        <View style={styles.card}>
          {quoteLoading ? (
            ...
            <TouchableOpacity style={styles.quoteErrorRow} onPress={loadQuote}>
              <Feather name="alert-circle" size={16} color="#DC2626" />
              <Text style={styles.quoteErrorText}>Could not calculate total. Tap to retry.</Text>
            </TouchableOpacity>
          ) : (
            <>
              <View style={styles.priceRow}>
                <Text style={styles.priceLabel}>Subtotal</Text>
                <Text style={styles.priceValue}>{money(displaySubtotal)}</Text>
              </View>

              <View style={styles.priceRow}>
                <Text style={styles.priceLabel}>Tax</Text>
                <Text style={styles.priceValue}>{money(displayTax)}</Text>
              </View>
              ...
              <View style={[styles.priceRow, styles.totalRow]}>
                ...
                <Text style={styles.totalValue}>{money(displayTotal)}</Text>
              </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[styles.payButton, (loading || locationLoading || quoteLoading || !quote) && styles.payButtonDisabled]}
          onPress={handlePay}
          disabled={loading || locationLoading || quoteLoading}
          activeOpacity={0.85}
        >
          {loading ? (
            <ActivityIndicator color="#FFF" />
          ) : locationLoading || quoteLoading ? (
            <View style={styles.payButtonInner}>
              <ActivityIndicator color="#FFF" size="small" />
              <Text style={styles.payButtonText}>Calculating...</Text>
            </View>
          ) : (
            <View style={styles.payButtonInner}>
              <Feather name="lock" size={18} color="#FFF" />
              <Text style={styles.payButtonText}>
                {quote ? `Pay ${money(displayTotal)}` : 'Calculate Total'}
              </Text>
            </View>
          )}
        </TouchableOpacity>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <SafeAreaView style={styles.modalContainer} edges={['top', 'bottom']}>
            ...
            <View style={styles.modalAmount}>
              ...
              <Text style={styles.modalAmountValue}>{money(displayTotal)}</Text>
            </View>
