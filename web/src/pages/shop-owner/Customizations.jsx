...
import {
  ...
  Layers, GripVertical, Zap, Sparkles
} from 'lucide-react';
...
function GroupCard({ group, onSave, onDelete, delay }) {
  ...
  const addOption = () => {
    ...
    setDraft((d) => ({
      ...
      options: [
        ...(d.options || []),
        { id: `new-${Date.now()}`, name: newOptionName.trim(), price_adjustment: 0, is_active: true },
      ],
    }));
    setNewOptionName('');
  };
  ...
  return (
    <motion.div
      ...
      whileHover={!editing ? { scale: 1.005 } : {}}
    ...
    >
      ...
      >
        {/* Icon */}
        <div className="p-3 bg-gradient-to-br from-amber-500 to-orange-500 rounded-xl shadow-lg shrink-0 group-hover:scale-110 transition-transform">
        ...
        <div className="flex-1 min-w-0">
          ...
          {editing ? (
            <div className="flex items-center gap-4 mt-2" onClick={(e) => e.stopPropagation()}>
              <label className="flex items-center gap-2 text-sm">
                ...
                <select
                  ...
                  value={draft.max_selections === 1 ? 'single' : 'multi'}
                  onChange={(e) => setDraft((d) => ({
                    ...d,
                    max_selections: e.target.value === 'single' ? 1 : null,
                    min_selections: e.target.value === 'single' ? 1 : 0,
                  }))}
                >
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {group.max_selections === 1 ? 'Single select' : 'Multi select'}
            ...
            {displayOptions?.map((opt, idx) =>
              editing ? (
                <OptionRow
                  key={opt.id || idx}
                  option={opt}
                  onUpdate={(u) => updateOption(idx, u)}
                  onDelete={() => removeOption(idx)}
                />
              ) : (
                <div
                  key={opt.id || idx}
                  className="flex justify-between text-sm p-2.5 bg-gray-50 dark:bg-neutral-800 rounded-xl"
                >
                  <span className="text-gray-800 dark:text-gray-200 font-medium">{opt.name}</span>
                  <span className="font-bold text-green-600">
                    {opt.price_adjustment > 0 ? `+$${parseFloat(opt.price_adjustment).toFixed(2)}` : 'Free'}
                  </span>
                </div>
              )
            )}
...
export default function Customizations() {
  ...
  const loadGroups = async () => {
    ...
    try {
      ...
      setGroups(
        (groupData || []).map((g) => ({
          ...g,
          options: (optData || []).filter((o) => o.modifier_group_id === g.id),
        }))
      );
    } catch (err) {
  ...
  const handleSaveGroup = async (draft) => {
    try {
      ...
      if (toDelete.length) {
        await supabase.from('modifier_options').update({ is_active: false }).in('id', toDelete);
      }

      for (const opt of draft.options) {
        if (opt.id?.startsWith('new-')) {
          await supabase.from('modifier_options').insert({
            modifier_group_id: draft.id,
            shop_id: shopId,
            name: opt.name,
          ...
          await supabase.from('modifier_options')
            .update({ name: opt.name, price_adjustment: opt.price_adjustment ?? 0 })
            .eq('id', opt.id);
        }
      }
      ...
      >
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            {groups.length} group{groups.length !== 1 ? 's' : ''} · reusable options for any item or POS
          </p>
        </div>
        <motion.button
          whileHover={{ scale: 1.05 }}
      ...
      {groups.length === 0 && !creating ? (
        ...
        >
          ...
          <p className="text-gray-500 mb-6 max-w-md mx-auto">
            Create groups like <strong>Milk Options</strong>, <strong>Syrup Flavors</strong>, or <strong>Size</strong> — then attach them to any menu item. Connect your POS to import automatically.
          </p>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
