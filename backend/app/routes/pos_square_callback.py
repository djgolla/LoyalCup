...
async def square_callback(request: Request, db=Depends(get_supabase)):
    ...
    # Always use/generate a real UUID for shop_id (never "test")
    try:
    ...
    # Step 1: Ensure shop exists in shops table (fill in not-null columns!)
    try:
        ...
        if not shop:
            # VVV -- Fill in ALL required fields for your schema here!
            shop_insert = {
                "id": shop_uuid,
                "name": "Square Connected Shop",  # REQUIRED
                # Add more required/default fields as needed here!
                "created_at": None,
                "updated_at": None
            }
