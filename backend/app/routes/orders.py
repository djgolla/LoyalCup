...
async def get_customer_orders(
    ...
    """Get customer's order list (basic info)"""
    try:
        customer_id = user.get("sub")
...
async def get_customer_order_history(
    ...
    """Get customer's full order history with shop names and items"""
    try:
        customer_id = user.get("sub")
        ...
        # Use dedicated method that includes shop info and items
...
async def get_shop_orders(
    ...
    try:
        ...
        # Use the dedicated get_shop_orders method which includes customer info and items
...
async def update_order_status(
    ...
    try:
        ...
        if order.get("shop_id") != shop_id:
            raise HTTPException(
                status_code=403, detail="Order does not belong to this shop"
            )
...
async def get_order_history(
    ...
    try:
        ...
        # Get orders
        orders = await order_service.list_orders(
            shop_id=shop_id, status=status, limit=per_page * page  # Simple pagination
        )
        ...
        return {
            ...
            "pagination": {"page": page, "per_page": per_page, "total": len(orders)},
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def export_orders(
    ...
    try:
        ...
        orders = await order_service.list_orders(
            shop_id=shop_id, limit=10000  # High limit for export
        )
