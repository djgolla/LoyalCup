...
async def connect_pos(req: ConnectRequest):
    ...
    url = adapters[req.provider].get_authorization_url(req.shop_id, req.redirect_uri, state)
...
async def exchange_code(req: ExchangeRequest, db: SupabaseClient = Depends(get_supabase)):
    ...
    # naive upsert: try select then update/insert (Supabase python upsert differs across libs)
    existing = await db.execute_query("pos_connections", "select", filters={"shop_id": req.shop_id, "provider": req.provider})
    if existing:
        await db.execute_query("pos_connections", "update", data=upsert, filters={"id": existing[0]["id"]})
    else:
...
async def list_locations(
    ...
    conns = await db.execute_query("pos_connections", "select", filters={"shop_id": shop_id, "provider": provider})
    if not conns or not conns[0].get("access_token"):
        raise HTTPException(status_code=400, detail="POS not connected")

    return {"locations": [loc.__dict__ for loc in await adapter.list_locations(conns[0]["access_token"])]}
...
async def sync_catalog(
    ...
    snapshot = await adapter.fetch_catalog(conns[0]["access_token"], location_id or conns[0].get("location_id"))

    # For today: return snapshot. Next: cache into your existing menu tables.
    return {
